...
// One task's delete action, the scheduled-task counterpart of
// DeleteAlertRuleDialog: destructive and irreversible (deletes the
// task's execution history too, via the server's own ON DELETE CASCADE),
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            This stops the schedule immediately and removes its execution
            ...
            onClick={() => {
              deleteTask.mutate(task.id, {
                onSuccess: () => {
                  setOpen(false)
                  toast.add({
                    title: `Scheduled task "${task.name}" deleted.`,
                    type: 'success',
                  })
                },
              })
