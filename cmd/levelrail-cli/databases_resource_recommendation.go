...
func runDatabasesResourceRecommendation(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "databases resource-recommendation", "print the recommendation as JSON to stdout and nothing else", stderr)
	fs.Usage = func() {
	...
	client, name, jsonOut, of, exitCode, ok := parseSingleArgClient(fs, args, apiFlagPtrs{tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP}, stderr, singleArgCmd{prog, "databases resource-recommendation", "database name"}, lookupEnv)
	if !ok {
		return exitCode
	}
	...
	return writeScheduledTaskResult(stdout, stderr, of, rec, func() { printResourceRecommendationHuman(stdout, "database", rec) })
}
