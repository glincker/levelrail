...
func runDatabases(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	...
	case "restore-upload":
		return runDatabasesRestoreUpload(prog, args[1:], stdout, stderr, lookupEnv)
...
func databasesUsage(prog string) string {
  ...
  %[1]s databases restore-upload <name> <file-path> [flags]  restore from a dump file on this machine
