...
func runDatabases(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	...
	case "init-scripts":
		return runDatabasesInitScripts(prog, args[1:], stdout, stderr, lookupEnv, os.Stdin)
	default:
...
func databasesUsage(prog string) string {
  ...
  %[1]s databases init-scripts list/create/update/delete <name> [flags]  named SQL/shell files run once on first container start (postgres/mysql/mariadb/mongodb only)
