...
// today); this command does not maintain its own copy of that list.
func runAppsMetrics(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "apps metrics", "print the queried points as JSON to stdout and nothing else", stderr)
	...
	fs.Usage = func() { _, _ = fmt.Fprint(stderr, appsMetricsUsage(prog)) }
	...
	name, ok := requireOneArg(fs, stderr, prog, "apps metrics", "app name")
	if !ok {
		return exitUsage
	}
	...
	result, err := client.QueryAppMetrics(context.Background(), name, metric, fromTime, toTime, stepDuration)
	if err != nil {
		return reportError(stdout, stderr, jsonOut, fmt.Errorf("query metrics for app %q: %w", name, err))
	}
