...
func runAppsGroup(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "apps group", "print the app group as JSON to stdout and nothing else", stderr)
	fs.Usage = func() {
	...
	client, name, jsonOut, of, exitCode, ok := parseSingleArgClient(fs, args, apiFlagPtrs{tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP}, stderr, singleArgCmd{prog, "apps group", "app name"}, lookupEnv)
	if !ok {
		return exitCode
	}
	...
	return writeScheduledTaskResult(stdout, stderr, of, group, func() { printAppGroupHuman(stdout, group) })
}
