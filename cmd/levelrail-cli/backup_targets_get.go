...
func runBackupTargetsGet(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "backup-targets get", "print the backup target as JSON to stdout and nothing else", stderr)
	...
	if err := renderResult(stdout, of.Format, of.Query, target, func() { printBackupTargetHuman(stdout, target) }); err != nil {
