...
func runChannelsTest(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "channels test", "print {\"sent\": true} as JSON to stdout on success and nothing else", stderr)
	...
	if err := renderResult(stdout, of.Format, of.Query, map[string]bool{"sent": true}, func() {
