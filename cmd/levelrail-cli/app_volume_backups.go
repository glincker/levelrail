...
func runAppVolumeBackups(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	...
	case "delete":
		return runAppVolumeBackupsDelete(prog, args[1:], stdout, stderr, lookupEnv)
	case "download":
		return runAppVolumeBackupsDownload(prog, args[1:], stdout, stderr, lookupEnv)
	case "restore":
...
func appVolumeBackupsUsage(prog string) string {
  ...
  %[1]s app-volume-backups delete <app> <volume> <backup-id> [flags]                  delete one archived backup on demand (destructive)
  %[1]s app-volume-backups download <app> <volume> <backup-id> [flags]                stream a succeeded backup's own object to stdout
