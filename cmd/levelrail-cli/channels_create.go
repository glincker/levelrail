...
// --kind pagerduty accepts --pagerduty-routing-key, and --kind resend
// accepts --resend-api-key/--resend-to/--resend-from, all convenience
// aliases so an operator doesn't have to know the API expects them
// packed into --notify-url (mirroring the web dashboard's connect
// dialog fields).
func runChannelsCreate(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	...
	var resendAPIKey, resendTo, resendFrom string
	var disabled bool
	fs.StringVar(&name, "name", "", "display name for the channel (required)")
	fs.StringVar(&kind, "kind", "", "channel kind: generic, slack, discord, telegram, email, pushover, pagerduty, teams, resend, ntfy, gotify, mattermost, lark (required)")
	...
	if resolvedURL == "" {
		return reportError(stdout, stderr, jsonOut, newValidationError("--notify-url is required (or, for --kind pushover, both --pushover-user-key and --pushover-api-token; or, for --kind pagerduty, --pagerduty-routing-key; or, for --kind resend, both --resend-api-key and --resend-to)"))
	}
...
func channelsCreateUsage(prog string) string {
  ...
  %[1]s channels create --name NAME --kind resend --resend-api-key KEY --resend-to EMAIL [flags]

Connects a new notification channel.

Flags:
  --name string                        display name for the channel (required)
  --kind string                        channel kind: generic, slack, discord, telegram, email, pushover, pagerduty, teams, resend, ntfy, gotify, mattermost, lark (required)
