...
func runUsersList(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	fs, tokenFlagP, apiURLFlagP, profileFlagP, jsonOutP, outputFlagP, queryFlagP := apiFlagSet(prog, "users list", "print users as a JSON array to stdout and nothing else", stderr)
	...
	if err := renderResult(stdout, of.Format, of.Query, users, func() { printUsersTable(stdout, users) }); err != nil {
