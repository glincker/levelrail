...
patches the same way, a node_disk_space rule watches every node's disk
usage percentage the same way too, and a node_resource_usage rule
watches every node's summed CPU and memory usage the same way as well,
...
overdue grace period, default 6h). A control_plane_backup_stale rule
watches the control plane's own newest snapshot platform-wide and fires
once it is older than --for-duration (default 3d); it stays quiet when
scheduled control plane backups are disabled. All ten notify the same
way once they fire.
...
func validateAppsAlertsCreateKind(kind, metric, comparator string, restartCountThreshold int, restartWindow, scheduledTaskID, backupResourceKind, backupDatabaseName, backupVolumeName string) error {
	...
	case "":
		return newValidationError("--kind is required (threshold, crashloop, cert_expiry, patch_status, scheduled_task_failure, node_disk_space, node_resource_usage, domain_health, backup_missing, or control_plane_backup_stale)")
	default:
		return newValidationError("--kind %q is not valid: must be threshold, crashloop, cert_expiry, patch_status, scheduled_task_failure, node_disk_space, node_resource_usage, domain_health, backup_missing, or control_plane_backup_stale", kind)
	}
	return nil
}
...
func runAppsAlertsCreate(prog string, args []string, stdout, stderr io.Writer, lookupEnv func(string) (string, bool)) int {
	...
	fs.StringVar(&kind, "kind", "", "rule kind: threshold, crashloop, cert_expiry, patch_status, scheduled_task_failure, node_disk_space, node_resource_usage, domain_health, backup_missing, control_plane_backup_stale (required)")
...
(cert_expiry watches every certificate on the control plane, patch_status
watches every node's pending OS security patches, node_disk_space watches
every node's disk usage percentage, node_resource_usage watches every
node's summed CPU and memory usage, none limited to <app>'s own domains
or workloads); <app> only decides where the rule shows up in "apps alerts
...
Flags:
  ...
  --kind string                        threshold, crashloop, cert_expiry, patch_status, scheduled_task_failure, node_disk_space, node_resource_usage, domain_health, backup_missing, or control_plane_backup_stale (required)
