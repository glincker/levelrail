...
// APP_API_TOKEN, then this server's own credentials file, then the CLI's.
func resolveToken(flagToken string, lookupEnv func(string) (string, bool), prog string) string {
	...
	for _, p := range []string{prog, cliProg} {
		if creds, err := apiclient.ReadCredentialsFile(p); err == nil && creds.Token != "" {
			return creds.Token
		}
	}
	return ""
...
func resolveAPIURL(flagURL string, lookupEnv func(string) (string, bool), prog string) string {
	...
	for _, p := range []string{prog, cliProg} {
		if creds, err := apiclient.ReadCredentialsFile(p); err == nil && creds.APIURL != "" {
			return creds.APIURL
		}
	}
