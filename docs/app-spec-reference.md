...
databases:
  main:
    engine: postgres          # postgres | redis | mysql | mongodb | mariadb | keydb | clickhouse
    version: "16"
...
| Field | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `engine` | string | yes | none | One of `postgres`, `redis`, `mysql`, `mongodb`, `mariadb`, `keydb`, `clickhouse`. |
