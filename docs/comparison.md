...
| MCP server / AI-agent-facing API layer | Yes, `cmd/levelrail-mcp`, 45 tools, same bearer-token/ability model as the REST API and CLI | Yes, roughly the same size: `app/Mcp/Servers/CoolifyServer.php`, roughly 45 tools, gated per-team by a settings flag | No; zero matches for `mcp` in the repo | No | No, and no REST API at all by design | No; no HTTP API of any kind |
...
an architectural difference it can shrug off: the template catalog (81
...
and notification channel breadth, both named as gaps in an earlier pass
of this table, have since closed: Levelrail now ships 45 MCP tools
(roughly matching Coolify's own count) and 13 notification channel
kinds, ahead of Dokploy's 12. Dokploy's fine-grained RBAC, self-service
invites, and per-request audit log are also real, working functionality,
worth naming plainly rather than waving off, though all three sit behind
a paid enterprise license upstream rather than Dokploy's free tier,
which matters for an apples-to-apples read. Levelrail's own invite model
closed part of that gap too (an admin-created email invite flow), though
it remains admin-initiated rather than Dokploy's self-service-among-
members shape.
