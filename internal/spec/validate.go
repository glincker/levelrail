...
func (s *Spec) Validate() error {
	...
	for name, db := range s.Databases {
		...
		if db.Engine != EnginePostgres && db.Engine != EngineRedis {
			return fmt.Errorf("spec: database %q: engine %q is not supported (Phase 1 supports %q and %q)", name, db.Engine, EnginePostgres, EngineRedis)
		}
	}
