...
// (cert_expiry.go), every node's patch status (patch_status.go), every
// node's disk usage (disk_space.go), or every node's CPU/memory usage
...
// The thirteen payload shapes NewNotifier knows how to build; an unknown
