...
// ControlPlaneDRSource is an optional extension of ControlPlaneBackupSource:
// DRProblem describes a failed or overdue off-box backup or restore drill, or
// returns "" when healthy.
type ControlPlaneDRSource interface {
	DRProblem(now time.Time) string
}
...
func EvaluateControlPlaneBackupStale(source ControlPlaneBackupSource, r Rule, defaultMaxAge time.Duration, now time.Time) (Rule, string, error) {
	...
	var drProblem string
	if dr, isDR := source.(ControlPlaneDRSource); isDR {
		drProblem = dr.DRProblem(now)
	}
	if !ok && drProblem == "" {
	...
	if !ok {
		return advanceState(next, r, true, 0, now), drProblem, nil
	}

	age := now.Sub(newest)
	v := age.Hours()
	next.LastValue = &v
	firing := age > maxAge || drProblem != ""

	var notice string
	switch {
	case drProblem != "":
		notice = "control plane: " + drProblem
	case firing:
