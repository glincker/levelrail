...
func newTestEngineWithTasks(rules *fakeRuleStore, metrics MetricsSource, logs LogsSource, tracker *RestartTracker, tasks ScheduledTaskSource, spy *spyNotifier) *Engine {
	return NewEngine(rules, metrics, logs, tracker, nil, tasks, 0, 0, nil, 0, 0, nil, 0, 0, nil, nil, 0, func(Rule) Notifier { return spy }, nil)
}
...
func newTestEngineWithDomainHealth(rules *fakeRuleStore, apps AppDomainSource, checker DomainCheckSource, checkInterval time.Duration, spy *spyNotifier) *Engine {
	return NewEngine(rules, nil, nil, nil, nil, nil, 0, 0, nil, 0, 0, nil, 0, 0, apps, checker, checkInterval, func(Rule) Notifier { return spy }, nil)
}
...
func TestEngine_Tick_MixedLegacyAndChannelRules_BothDispatch(t *testing.T) {
	...
	metrics := &fakeMetricsSource{samples: []telemetry.Sample{{Timestamp: time.Now(), Value: 95}}}
	spy := &spyNotifier{}
	engine := NewEngine(db, metrics, nil, nil, nil, nil, 0, 0, nil, 0, 0, nil, 0, 0, nil, nil, 0, func(Rule) Notifier { return spy }, nil)
...
func TestEngine_Dispatch_RecordsDeliveryForChannelAttachedRule(t *testing.T) {
	...
	metrics := &fakeMetricsSource{samples: []telemetry.Sample{{Timestamp: time.Now(), Value: 95}}}
	engine := NewEngine(db, metrics, nil, nil, nil, nil, 0, 0, nil, 0, 0, nil, 0, 0, nil, nil, 0, func(Rule) Notifier { return &spyNotifier{} }, nil)
