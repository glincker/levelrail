...
// TestDriver_OwnsPort_DirectMapAccess exercises OwnsPort's own
// mutex-guarded map read directly, without going through a real Caddy
// Apply/Stop: TestDriver_OwnsPort below is the real end-to-end proof but
// skips under -short (skipUnderLoad, a known upstream Caddy data race
// under concurrent CI load), so this is what keeps New and OwnsPort's own
// logic covered under a short-mode run too.
func TestDriver_OwnsPort_DirectMapAccess(t *testing.T) {
	d := New(testLogger(t))
	if d.OwnsPort(443) {
		t.Error("OwnsPort(443) = true on a fresh Driver, want false")
	}

	d.mu.Lock()
	d.listenPorts = map[int]bool{443: true}
	d.mu.Unlock()

	if !d.OwnsPort(443) {
		t.Error("OwnsPort(443) = false after directly setting listenPorts, want true")
	}
	if d.OwnsPort(8080) {
		t.Error("OwnsPort(8080) = true for a port never in listenPorts, want false")
	}
}
