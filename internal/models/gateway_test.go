...
func TestGateway(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "dot dot escapes v1", host: "chat.example.com", path: "/v1/../api/pull", auth: "Bearer " + key, wantCode: 404, handled: true},
		{name: "encoded dot dot escapes v1", host: "chat.example.com", path: "/v1/%2e%2e/api/pull", auth: "Bearer " + key, wantCode: 404, handled: true},
		{name: "dot dot inside v1 still rejected", host: "chat.example.com", path: "/v1/models/../chat/completions", auth: "Bearer " + key, wantCode: 404, handled: true},
		{name: "double slash normalises away from v1", host: "chat.example.com", path: "/v1//../../api/tags", auth: "Bearer " + key, wantCode: 404, handled: true},
