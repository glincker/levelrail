...
func (rt *Router) registerPipelineRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/v1/apps/{name}/pipelines", rt.requireAbility(AbilityRead, rt.handleListPipelines))
	...
	mux.HandleFunc("GET /api/v1/apps/{name}/pipelines/{pname}", rt.requireAbility(AbilityRead, rt.handleGetPipeline))
	...
	mux.HandleFunc("GET /api/v1/apps/{name}/pipeline-runs", rt.requireAbility(AbilityRead, rt.handleListPipelineRuns))
	mux.HandleFunc("GET /api/v1/apps/{name}/pipeline-runs/{id}", rt.requireAbility(AbilityRead, rt.handleGetPipelineRun))
	mux.HandleFunc("GET /api/v1/apps/{name}/pipeline-runs/{id}/logs", rt.requireAbility(AbilityRead, rt.handleListPipelineRunLogs))
	mux.HandleFunc("GET /api/v1/apps/{name}/pipeline-runs/{id}/logs/stream", rt.requireAbility(AbilityRead, rt.handleStreamPipelineRunLogs))
