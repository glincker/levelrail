...
func (m *mockAuditStore) DeleteAuditEntriesOlderThan(_ context.Context, cutoff time.Time) (int64, error) {
	m.deletedOlderThan = cutoff
	m.callCount++
	return m.returnedDeleted, m.returnedError
}

func (m *mockAuditStore) SaveAuditEntry(_ context.Context, _ store.AuditEntry) error {
	return nil
}

func (m *mockAuditStore) ListAuditEntries(_ context.Context, _ int, _ *time.Time, _ store.AuditEntryFilter) ([]store.AuditEntry, error) {
	return nil, nil
}
