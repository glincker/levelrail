...
func TestHandleGetSession_EmailDefaultsEmpty(t *testing.T) {
	rt, db := newTestRouter(t)
	cookie := loginTestSession(t, rt, db)
	...
	if got.Email != "" {
		t.Errorf("Email = %q, want empty (no recovery email on file by default)", got.Email)
	}
}

func TestHandleUpdateAdminEmail_Success(t *testing.T) {
	rt, db := newTestRouter(t)
	cookie := loginTestSession(t, rt, db)

	rec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(rec, authedRequest(t, cookie, http.MethodPut, "/api/v1/auth/email", `{"email":"ops@example.com"}`))
	if rec.Code != http.StatusNoContent {
		t.Fatalf("status = %d, want %d, body = %s", rec.Code, http.StatusNoContent, rec.Body.String())
	}
	...
	if admin.Email != "ops@example.com" {
		t.Errorf("Email = %q, want ops@example.com", admin.Email)
	}

	sessRec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(sessRec, authedRequest(t, cookie, http.MethodGet, "/api/v1/auth/session", ""))
	var got sessionInfoResponse
	if err := json.Unmarshal(sessRec.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode: %v", err)
	}
	if got.Email != "ops@example.com" {
		t.Errorf("GET /auth/session Email = %q, want ops@example.com", got.Email)
	}
}

func TestHandleUpdateAdminEmail_CanClearToEmpty(t *testing.T) {
	rt, db := newTestRouter(t)
	cookie := loginTestSession(t, rt, db)

	rt.Handler().ServeHTTP(httptest.NewRecorder(), authedRequest(t, cookie, http.MethodPut, "/api/v1/auth/email", `{"email":"ops@example.com"}`))

	rec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(rec, authedRequest(t, cookie, http.MethodPut, "/api/v1/auth/email", `{"email":""}`))
	if rec.Code != http.StatusNoContent {
		t.Fatalf("status = %d, want %d, body = %s", rec.Code, http.StatusNoContent, rec.Body.String())
	}
	...
	if admin.Email != "" {
		t.Errorf("Email = %q, want empty after clearing", admin.Email)
	}
}

func TestHandleUpdateAdminEmail_InvalidFormatRejected(t *testing.T) {
	rt, db := newTestRouter(t)
	cookie := loginTestSession(t, rt, db)

	rec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(rec, authedRequest(t, cookie, http.MethodPut, "/api/v1/auth/email", `{"email":"not-an-email"}`))
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want %d, body = %s", rec.Code, http.StatusBadRequest, rec.Body.String())
	}
	...
	if admin.Email != "" {
		t.Errorf("Email = %q, want unchanged (empty) after a rejected update", admin.Email)
	}
}

func TestHandleUpdateAdminEmail_RequiresSession(t *testing.T) {
	rt, _ := newTestRouter(t)

	rec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(rec, httptest.NewRequest(http.MethodPut, "/api/v1/auth/email", strings.NewReader(`{"email":"ops@example.com"}`)))
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
}

func TestHandleUpdateAdminEmail_DoesNotRequireCurrentPassword(t *testing.T) {
	rt, db := newTestRouter(t)
	cookie := loginTestSession(t, rt, db)

	// No current_password field at all: unlike handleChangePassword,
	// setting the recovery email must not require re-supplying a
	// credential, per handleUpdateAdminEmail's own doc comment.
	rec := httptest.NewRecorder()
	rt.Handler().ServeHTTP(rec, authedRequest(t, cookie, http.MethodPut, "/api/v1/auth/email", `{"email":"ops@example.com"}`))
	if rec.Code != http.StatusNoContent {
		t.Fatalf("status = %d, want %d, body = %s", rec.Code, http.StatusNoContent, rec.Body.String())
